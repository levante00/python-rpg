...
class MoveInChain(AbstractHandler):
	def handle(self, request) -> None: 
		if (request == "N" or request == "S" or
			...
			room_cell =  self.MC.current_room.interior[
						 self.MC.room_position_y][self.MC.room_position_x]
...
class Help(Command):
	def __init__(self) -> None:
		...
		self.text_2 = ('After Calling "Map" Command You Will See Blocks With Cells\nEach of Theme'
...
class ShowMap(Command):
	...
	def execute(self) -> None:
		Room.show_map(
			self.Map, self.MC.current_room.global_position_x,
			self.MC.current_room.global_position_y, self.MC.room_position_x,
			self.MC.room_position_y)
