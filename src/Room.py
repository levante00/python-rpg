...
class Room:
    def __init__(
        self,
        name: str,
        global_position_x: int,
        global_position_y: int,
        music: str,
        interior: list = [],
        monsters: list = [],
        items: list = [],
        ...
        self.global_position_x = (
            global_position_x  # X Coordinate Relative to Other Rooms
        )
        self.global_position_y = (
            global_position_y  # Y Coordintae Relative to Other Rooms
        )
        self.music = music
    ...
    def generate_monsters(self, diff: str) -> None:
        ...
        self.monsters = random.choices(
            Enemy.bestiary, weights=monster_weights, k=random.randint(2, monster_number)
        )
        ...
        for i in range(self.size):
            for j in range(self.size):
                if isinstance(self.interior[i][j], Enemy.Monster):
                    print(
                        Fore.RED + f"{self.interior[i][j].name[0]}",
                        Style.RESET_ALL,
                        end="",
                    )
                elif isinstance(self.interior[i][j], Items.Item):
                    print(
                        Fore.BLUE + f"{self.interior[i][j].name[0]}",
                        Style.RESET_ALL,
                        end="",
                    )
                    ...
                    for e in range(row_len):
                        for j in range(-1, size + 1):
                            if (j == -1 and i != -1) or (j == size and i != -1):
                                ...
                                Map[k + e].global_position_x == global_position_x
                                and Map[k + e].global_position_y == global_position_y
                                and i == room_position_y
                                and j == room_position_x
                            ):
